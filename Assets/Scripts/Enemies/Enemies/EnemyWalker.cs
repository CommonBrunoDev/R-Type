...
{
    public Bullet[] bulletMagazine = new Bullet[5];
    public Bullet bulletRef;

    public float[] shootDelayMinMax = new float[2];
    ...
    void Update()
    {
        ...
        if (GameManager.Instance.gameState != GameState.Paused)
        {
            ...
            {
                ShootBullet();
                shootTimer = Random.RandomRange(shootDelayMinMax[0], shootDelayMinMax[1]);
            }
    ...
    void ShootBullet()
    {
        ...
        while (check < 99)
        {
            ...
            if (!bulletMagazine[rnd].gameObject.activeSelf)
            {
                ...
                bulletMagazine[rnd].gameObject.transform.parent = transform.parent;
                bulletMagazine[rnd].gameObject.transform.position = transform.position;
                bulletMagazine[rnd].gameObject.GetComponentInChildren<SpriteRenderer>().transform.rotation = Quaternion.Euler(new Vector3(0, 0, 90));
                bulletMagazine[rnd].gameObject.GetComponent<Bullet>().direction = Vector3.up;
